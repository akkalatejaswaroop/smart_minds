...
const BookSummarizerView: React.FC = () => {
    ...
    return (
        <div className="flex-1 flex flex-col p-4 sm:p-6 overflow-y-auto">
            ...
            {!submittedBook ? (
                <div className="bg-slate-800/50 border border-slate-700/50 rounded-sm p-6 max-w-3xl mx-auto w-full">
                    <div className="flex border-b border-slate-700/50 mb-4">
                        <button onClick={() => setAnalysisMode('title')} className={`px-4 py-2 text-sm font-medium transition-colors ${analysisMode === 'title' ? 'border-b-2 border-cyan-500 text-white' : 'text-slate-400 hover:bg-slate-800/50'}`}>Analyze by Title</button>
                        <button onClick={() => setAnalysisMode('file')} className={`px-4 py-2 text-sm font-medium transition-colors ${analysisMode === 'file' ? 'border-b-2 border-cyan-500 text-white' : 'text-slate-400 hover:bg-slate-800/50'}`}>Analyze from File</button>
                    </div>

                    {analysisMode === 'title' ? (
                        <div className="flex flex-col sm:flex-row gap-2">
                            <input type="text" value={bookInput} onChange={e => setBookInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAnalyze()} placeholder="e.g., 'The Great Gatsby' or 'Sapiens'" className="flex-grow bg-slate-800 border border-slate-600 rounded-sm px-4 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:outline-none"/>
                            <button onClick={handleAnalyze} disabled={isAnalyzeDisabled} className="bg-cyan-600 text-white font-semibold px-5 py-2 rounded-sm hover:bg-cyan-500 transition-colors disabled:bg-cyan-900/50 disabled:text-slate-400 disabled:cursor-not-allowed flex items-center justify-center">Analyze</button>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            <label htmlFor="file-upload" className="relative block w-full border-2 border-dashed border-slate-600 rounded-sm p-8 text-center cursor-pointer hover:border-cyan-500 transition-colors">
                            ...
                            {(uploadedFile || isFileReading) && (
                                <div className="flex items-center justify-center gap-4">
                                    <button onClick={handleAnalyze} disabled={isAnalyzeDisabled} className="bg-cyan-600 text-white font-semibold px-5 py-2 rounded-sm hover:bg-cyan-500 transition-colors disabled:bg-cyan-900/50 disabled:text-slate-400 disabled:cursor-not-allowed flex items-center justify-center">
                ...
                <>
                    <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
                        ...
                        <button onClick={handleReset} className="text-sm bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-sm flex-shrink-0">
                            Analyze Another
                        </button>
                    </div>

                    <div className="space-y-4 bg-slate-800/50 p-6 rounded-sm border border-slate-700/50">
                        <h3 className="text-xl font-semibold text-cyan-400">Generator Controls</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label className="text-xs font-medium text-slate-400 block mb-1">Insight Type</label>
                                <select value={insightType} onChange={e => setInsightType(e.target.value as any)} className="w-full bg-slate-800 border border-slate-700/50 rounded-sm px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:outline-none">
                            ...
                            <div>
                                ...
                                <select value={insightFormat} onChange={e => setInsightFormat(e.target.value as any)} className="w-full bg-slate-800 border border-slate-700/50 rounded-sm px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:outline-none" disabled={!['summary', 'concepts'].includes(insightType)}>
                            ...
                            <div>
                                ...
                                <input type="text" value={insightPurpose} onChange={e => setInsightPurpose(e.target.value)} placeholder="e.g., preparing for an exam" className="w-full bg-slate-800 border border-slate-700/50 rounded-sm px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:outline-none"/>
                            </div>
                        </div>
                        <button onClick={handleAnalyze} disabled={isGenerating} className="w-full bg-cyan-600 text-white font-semibold px-5 py-2 rounded-sm hover:bg-cyan-500 transition-colors disabled:bg-cyan-900/50 disabled:text-slate-400 flex items-center justify-center gap-2">
