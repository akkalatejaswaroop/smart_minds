...
const BookCard: React.FC<{ book: Book; onSelect: () => void }> = ({ book, onSelect }) => (
  <button onClick={onSelect} className="text-left group focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg">
  ...
  if (selectedBook) {
    ...
    return (
        <div className="flex-1 flex flex-col p-4 sm:p-6 bg-slate-900/50 overflow-y-auto">
            <button onClick={() => setSelectedBook(null)} className="flex items-center gap-2 text-sm text-indigo-400 hover:text-indigo-300 mb-4 self-start">
                ...
                <div className="flex-1 w-full">
                    <div className="flex border-b border-slate-700">
                        {(['read', 'summarize', 'details'] as const).map(tab => (
                            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium transition-colors capitalize ${activeTab === tab ? 'border-b-2 border-indigo-500 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>{tab}</button>
                        ))}
                    </div>
                    <div className="py-6">
                        {activeTab === 'read' && (
                            <div className="space-y-4">
                               <div className="flex justify-between items-center flex-wrap gap-4">
                                    <select value={currentChapterIndex} onChange={e => { setCurrentChapterIndex(parseInt(e.target.value)); setChapterQuiz(null); }} className="bg-slate-800 text-lg font-semibold text-purple-400 border border-slate-700 rounded-md px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                                      {selectedBook.chapters.map((chap, i) => <option key={i} value={i}>{chap.title}</option>)}
                                    </select>
                                    <div className="flex items-center gap-2">
                                        <select value={theme} onChange={e => setTheme(e.target.value)} className="bg-slate-800 text-xs border border-slate-700 rounded-md px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:outline-none"><option value="dark">Dark</option><option value="light">Light</option></select>
                                        <div className="flex items-center bg-slate-800 rounded-md border border-slate-700">
                                            <button onClick={() => setFontSize('text-sm')} className={`px-2 py-1 text-xs ${fontSize === 'text-sm' ? 'bg-indigo-700 text-white' : ''} rounded-l-md`}>A</button>
                                            <button onClick={() => setFontSize('text-base')} className={`px-2 py-1 text-sm ${fontSize === 'text-base' ? 'bg-indigo-700 text-white' : ''}`}>A</button>
                                            <button onClick={() => setFontSize('text-lg')} className={`px-2 py-1 text-base ${fontSize === 'text-lg' ? 'bg-indigo-700 text-white' : ''} rounded-r-md`}>A</button>
                                        </div>
                                    </div>
                               </div>
                                ...
                                <div className="mt-4">
                                  {!chapterQuiz ? (
                                      <button onClick={() => handleGenerateQuiz('chapter')} disabled={isGeneratingChapterQuiz} className="w-full bg-indigo-700 text-white font-semibold px-5 py-2 rounded-md hover:bg-indigo-600 transition-colors disabled:bg-indigo-900/50 flex items-center justify-center gap-2">
                        ...
                        {activeTab === 'summarize' && (
                            <div className="space-y-4">
                                <h3 className="text-xl font-semibold text-purple-400">AI Book Summarizer</h3>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label className="text-xs font-medium text-slate-400 block mb-1">Content</label>
                                        <select value={summaryChapter} onChange={e => setSummaryChapter(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"><option value="full">Full Book</option>{selectedBook.chapters.map((chap, i) => <option key={i} value={i}>{chap.title}</option>)}</select>
                                    </div>
                                    <div>
                                        <label className="text-xs font-medium text-slate-400 block mb-1">Format</label>
                                        <select value={summaryType} onChange={e => setSummaryType(e.target.value as any)} className="w-full bg-slate-800 border border-slate-700 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"><option value="paragraph">Paragraph</option><option value="bullets">Bullet Points</option><option value="concept-map">Concept Map</option></select>
                                    </div>
                                    <div>
                                        <label className="text-xs font-medium text-slate-400 block mb-1">Purpose</label>
                                        <input type="text" value={summaryPurpose} onChange={e => setSummaryPurpose(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"/>
                                    </div>
                                </div>
                                <button onClick={handleGenerateSummary} disabled={isSummarizing} className="w-full bg-indigo-700 text-white font-semibold px-5 py-2 rounded-md hover:bg-indigo-600 transition-colors disabled:bg-indigo-900/50 flex items-center justify-center gap-2">
                        ...
                        {activeTab === 'details' && (
                            <div className="space-y-6">
                                <div><h3 className="text-xl font-semibold text-purple-400 mb-2">Details</h3><div className="text-sm space-y-2"><p><strong>Genre:</strong> {selectedBook.genre}</p><p><strong>Level:</strong> {selectedBook.level}</p><p><strong>Est. Reading Time:</strong> {selectedBook.readingTime}</p></div></div>
                                <div><h3 className="text-xl font-semibold text-purple-400 mb-2">Summary</h3><p className="text-sm text-slate-400 leading-relaxed">{selectedBook.summary}</p></div>
                                <div><h3 className="text-xl font-semibold text-purple-400 mb-2">Download Full Book</h3><div className="flex gap-4"><button onClick={() => handleDownloadFullBook('pdf')} className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-md"><DownloadIcon /> PDF</button><button onClick={() => handleDownloadFullBook('docx')} className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-md"><DownloadIcon /> DOCX</button></div></div>
                                <div><h3 className="text-xl font-semibold text-purple-400 mb-2">Related Books (AI-Powered)</h3>{isLoadingRecs ? <p className="text-sm text-slate-400">Finding recommendations...</p> : (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">{recommendations.length > 0 ? recommendations.map(book => (<BookCard key={book.id} book={book} onSelect={() => setSelectedBook(book)} />)) : <p className='text-sm text-slate-500 col-span-full'>No recommendations found.</p>}</div>)}</div>
                            </div>
                        )}
                    </div>
                </div>
    ...
    <div className="flex-1 flex flex-col p-6 bg-slate-900/50 overflow-y-auto">
      ...
      <div className="relative mb-6">
        <input 
          ...
          className="w-full bg-slate-800 border border-slate-700 rounded-md py-2 pl-10 pr-12 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"
        />
        <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
        <button onClick={isListening ? stopListening : startListening} className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full ${isListening ? 'bg-indigo-500/20 text-indigo-400 animate-pulse' : 'text-slate-400 hover:text-white'}`}>
            <MicrophoneIcon />
        </button>
      </div>
