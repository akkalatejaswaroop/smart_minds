    ...
    <div className="flex-1 flex flex-col p-6 bg-slate-900/50 overflow-y-auto">
      ...
      <div className="relative mb-6">
        <input 
          ...
          className="w-full bg-slate-800 border border-slate-700 rounded-md py-2 pl-10 pr-12 text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none"
        />
        <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
        <button onClick={isListening ? stopListening : startListening} className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full ${isListening ? 'bg-teal-500/20 text-teal-400 animate-pulse' : 'text-slate-400 hover:text-white'}`}>
            <MicrophoneIcon />
        </button>
      </div>
