...
const ExplanationDisplay: React.FC<{ data: CodeExplanation }> = ({ data }) => (
    <div className="prose prose-invert max-w-none space-y-6">
        <div>
            <h3 className="text-cyan-400 border-b border-slate-700/50 pb-2">Summary</h3>
            <p className="mt-2 text-slate-300">{data.summary}</p>
        </div>
        <div>
            <h3 className="text-cyan-400 border-b border-slate-700/50 pb-2">Line-by-Line Explanation</h3>
            <dl className="mt-4 space-y-4">
                {data.lineByLineExplanation.map((item, index) => (
                    <div key={index} className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                        <dt className="font-mono text-cyan-400 flex-shrink-0 sm:w-24 sm:text-right">{item.lines}</dt>
        ...
        <div>
            <h3 className="text-cyan-400 border-b border-slate-700/50 pb-2">Key Concepts</h3>
...
const DebugDisplay: React.FC<{ data: CodeDebugReport }> = ({ data }) => (
    <div className="prose prose-invert max-w-none space-y-6">
        <div>
            <h3 className="text-cyan-400 border-b border-slate-700/50 pb-2">Analysis Summary</h3>
            <p className="mt-2 text-slate-300">{data.analysisSummary}</p>
        </div>
        
        {data.bugs.length > 0 && (
            <div>
                <h3 className="text-cyan-400 border-b border-slate-700/50 pb-2">Identified Issues</h3>
                <div className="mt-4 space-y-4">
                    {data.bugs.map((bug, index) => (
                        <div key={index} className="bg-slate-900/50 p-4 rounded-sm border border-slate-600">
        ...
        <div>
            <h3 className="text-cyan-400 border-b border-slate-700/50 pb-2">Corrected Code</h3>
            <div 
                className="prose prose-invert max-w-none prose-pre:bg-slate-900 prose-pre:p-4 prose-pre:rounded-sm prose-code:text-pink-400"
...
const CodeInputWithLineNumbers: React.FC<{
    ...
    return (
        <div 
            className="flex-1 flex bg-slate-900 border border-slate-700/50 rounded-sm overflow-hidden focus-within:ring-2 focus-within:ring-cyan-500"
        ...
        <div className="flex-1 flex flex-col p-4 sm:p-6 overflow-y-auto">
            ...
            <div className="flex-1 flex flex-col lg:flex-row gap-6">
                ...
                <div className="w-full lg:w-1/2 flex flex-col gap-4">
                    <div className="flex items-center bg-slate-800 rounded-sm p-1">
                        <button
                            onClick={() => setAction('explain')}
                            className={`flex-1 px-3 py-2 text-sm font-medium rounded-sm transition-colors flex items-center justify-center gap-2 ${action === 'explain' ? 'bg-cyan-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
                        ...
                        <button
                            ...
                            className={`flex-1 px-3 py-2 text-sm font-medium rounded-sm transition-colors flex items-center justify-center gap-2 ${action === 'debug' ? 'bg-cyan-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
                    ...
                    <button
                        ...
                        className="w-full bg-cyan-600 text-white font-semibold px-5 py-3 rounded-sm hover:bg-cyan-500 transition-colors disabled:bg-cyan-900/50 disabled:text-slate-400 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                ...
                <div className="w-full lg:w-1/2 flex flex-col">
                    <div className="flex-1 bg-slate-800 rounded-sm p-6 overflow-y-auto border border-slate-700/50 min-h-[400px]">
                        {isLoading && (
