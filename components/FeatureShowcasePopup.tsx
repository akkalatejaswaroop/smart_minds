...
const features: { icon: React.ReactElement; title: string; description: string; view: AppView }[] = [
  {
    icon: <BrainCircuitIcon className="w-8 h-8 text-indigo-400" />,
  ...
  {
    icon: <FileTextIcon className="w-8 h-8 text-indigo-400" />,
  ...
  {
    icon: <MessageSquareIcon className="w-8 h-8 text-indigo-400" />,
    title: 'AI Tutor Chat',
...
export const FeatureShowcasePopup: React.FC<FeatureShowcasePopupProps> = ({ isOpen, onClose, onSetView }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col transform animate-slide-up"
        onClick={e => e.stopPropagation()} // Prevent closing when clicking inside the modal
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-700 flex-shrink-0">
          <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-pink-500">
            Welcome to SMART MINDS!
          </h2>
          <button
            onClick={onClose}
            className="p-2 -mr-2 text-slate-400 rounded-full hover:bg-slate-700 hover:text-white"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {features.map((feature, index) => (
                    <button
                      ...
                      className="flex items-start text-left gap-4 p-4 bg-slate-800/50 rounded-lg hover:bg-slate-700/80 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
        ...
        <div className="p-4 border-t border-slate-700 text-center flex-shrink-0">
            <button
              onClick={onClose}
              className="bg-indigo-700 text-white font-semibold px-6 py-2 rounded-md hover:bg-indigo-600 transition-colors"
            >
                Get Started
            </button>
        </div>
